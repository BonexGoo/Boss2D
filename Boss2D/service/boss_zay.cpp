...
ZAY_DECLARE_VIEW("_default_view_")
...
{
    ZAY_RGB(panel, 0x80, 0xFF, 0x80) panel.fill();
    ZAY_RGB(panel, 0x00, 0x00, 0x00) panel.text("<DEFAULT VIEW>");
}
...
namespace BOSS
{
    ...
    typedef ZayArray<TouchRectObject> TouchRectObjects;
    ...
    public:
        static ZayMap<TouchCollector>& STMAP()
        {return *BOSS_STORAGE_SYS(ZayMap<TouchCollector>);}
    ...
    bool ZayPanel::textbox(chars string, sint32 linegap, UIAlign align) const
    {
        ...
        ZayArray<LineText, datatype_pod_canmemcpy> LineTexts;
    ...
    ZayExtend::Payload::Payload(const ZayExtend* owner, chars uiname, sint32 elementid, Solver* lvalue, const Renderer* renderer)
        : mElementID(elementid), mRefLValue(lvalue), mRefRenderer(renderer)
    {
        mOwner = owner;
        mUIName = uiname;
    }
    ...
    ZayExtend::Payload::Payload(const ZayExtend* owner, const SolverValue& param)
        : mElementID(-1), mRefLValue(nullptr), mRefRenderer(nullptr)
    {
        mOwner = owner;
        mUIName = nullptr;
        AddParam(param);
    ...
    // ZayView
    ////////////////////////////////////////////////////////////////////////////////
    ZayView::ZayView(chars viewclass) : m_viewfunc(*Function::GetForced(viewclass)),
        m_viewclass((viewclass)? viewclass : "")
    {
        m_data = m_viewfunc.m_alloc();
    ...
    void ZayView::DirtyAllSurfaces()
    {
        TouchCollector::STMAP().AccessByCallback(
            [](const ZayMapPath*, TouchCollector* collector, payload param)->void
            {
    ...
    void ZayView::OnCreate()
    {
        ...
        m_viewfunc.m_command(CT_Create, nullptr, nullptr);
        m_viewfunc.m_unlock();
    }
    ...
    bool ZayView::OnCanQuit()
    {
        if(!m_agreed_quit)
        {
            ...
            m_viewfunc.m_command(CT_CanQuit, nullptr, &out);
            m_viewfunc.m_unlock();
    ...
    {
        m_viewfunc.m_lock(m_data);
        m_viewfunc.m_command(CT_Destroy, nullptr, nullptr);
        m_viewfunc.m_unlock();
    ...
    {
        m_viewfunc.m_lock(m_data);
        m_viewfunc.m_command(CT_Activate, boolo(actived), nullptr);
        m_viewfunc.m_unlock();
    }
    ...
    void ZayView::OnSize(sint32 w, sint32 h)
    {
        ...
        m_viewfunc.m_lock(m_data);
        m_viewfunc.m_command(CT_Size, WH, nullptr);
        m_viewfunc.m_unlock();
    }
    ...
    {
        m_viewfunc.m_lock(m_data);
        m_viewfunc.m_command(CT_Tick, nullptr, nullptr);
        m_viewfunc.m_unlock();
    ...
    {
        m_viewfunc.m_lock(m_data);
        m_viewfunc.m_notify(type, topic, in, out);
        m_viewfunc.m_unlock();
    }
    ...
    void ZayView::OnTouch(TouchType type, sint32 id, sint32 x, sint32 y)
    {
        ...
        m_viewfunc.m_lock(m_data);
        m_viewfunc.m_command(CT_Touch, XY, nullptr);
        m_viewfunc.m_unlock();
    ...
    void ZayView::OnKey(sint32 code, chars text, bool pressed)
    {
        ...
        else
        {
            m_viewfunc.m_lock(m_data);
            if(pressed)
                m_viewfunc.m_notify(NT_KeyPress, text, sint32o(code), nullptr);
            else m_viewfunc.m_notify(NT_KeyRelease, text, sint32o(code), nullptr);
            m_viewfunc.m_unlock();
        }
    ...
    {
        static ZayMap<Function> AllFunctions;
    ...
    ZayView::Function* ZayView::Function::GetForced(chars viewclass)
    {
        Function* Result = ZayView::_accessfunc(viewclass, false);
        if(!Result)
            Result = ZayView::_accessfunc("_unmatched_view_", true);
        return Result;
    }
}
