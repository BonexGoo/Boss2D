...
public:
    ...
    public:
        ...
        /// @param url : REST-API식 url요청("https://a.b.com/ask?aa=0&bb=1")
        /// @param csv : 콤마와 줄바꿈으로 구성된 테이블데이터
        /// @param cb : 응답이 도착할 콜백함수
        /// @param data : 콜백함수에 전달할 데이터
        static void SendCsvRequest(chars url, chars csv, RequestEventCB cb, payload data = nullptr);
    ...
    public:
        ...
        /// @param p : 꼭지점들(px)
        /// @param curve : 커브율
        static void FillBezier(float x, float y, Points p, float curve);
        ...
        /// @param y : 상단위치(px)
        /// @param ps : 꼭지점들(px)
        /// @param colors : 컬러링값들(multiply블랜드값)
        /// @param fbo : 출력될 프레임버퍼의 FBO핸들(화면출력시 0)
        static void DrawPolyImageToFBO(id_image_read image, const Point (&ips)[3], float x, float y, const Point (&ps)[3], const Color (&colors)[3], uint32 fbo = 0);

        /// @brief 지정한 FBO핸들로 회전시킨 이미지 출력
        /// @param image : 출력할 이미지
        /// @param ox : 이미지의 회전중심(px)
        /// @param oy : 이미지의 회전중심(px)
        /// @param degree : 이미지의 회전값(360˚기준)
        /// @param x : 좌측위치(px)
        /// @param y : 상단위치(px)
        /// @param color : 컬러링값
        /// @param fbo : 출력될 프레임버퍼의 FBO핸들(화면출력시 0)
        static void DrawRotatedImageToFBO(id_image_read image, float ox, float oy, float degree, float x, float y, Color color = Color::White, uint32 fbo = 0);
