...
// removed by BOSS, original-code: namespace internal {
static int DetectNumberOfCores_BOSS() {// modified by BOSS, original-code: static int DetectNumberOfCores() {
...
uint32_t CpuInfo::DetectNumberOfCores() {
  ...
  if (!logical_cpus)
    logical_cpus = static_cast<uint32_t>(DetectNumberOfCores_BOSS());// modified by BOSS, original-code: logical_cpus = static_cast<uint32_t>(internal::DetectNumberOfCores());
  return logical_cpus;
}
