...
#include BOSS_WEBRTC_U_modules__rtp_rtcp__source__receive_statistics_impl_h //original-code:"modules/rtp_rtcp/source/receive_statistics_impl.h"
...
std::vector<rtcp::ReportBlock_rtcp_BOSS> ReceiveStatisticsImpl::RtcpReportBlocks( // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
    size_t max_blocks) {
  ...
  std::vector<rtcp::ReportBlock_rtcp_BOSS> result; // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
  ...
  auto add_report_block = [&result](uint32_t media_ssrc,
    ...
    if (!statistician->GetActiveStatisticsAndReset(&stats))
      return;
    result.emplace_back();
    rtcp::ReportBlock_rtcp_BOSS& block = result.back(); // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
