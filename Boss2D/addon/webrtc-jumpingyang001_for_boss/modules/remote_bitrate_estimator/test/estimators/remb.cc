...
#include BOSS_WEBRTC_U_modules__remote_bitrate_estimator__remote_bitrate_estimator_abs_send_time_h //original-code:"modules/remote_bitrate_estimator/remote_bitrate_estimator_abs_send_time.h"
...
FeedbackPacket* RembReceiver::GetFeedback(int64_t now_ms) {
  ...
  if (LatestEstimate(&estimated_bps)) {
    ...
    if (!report_blocks.empty()) {
      const rtcp::ReportBlock_rtcp_BOSS& stat = report_blocks.front(); // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
