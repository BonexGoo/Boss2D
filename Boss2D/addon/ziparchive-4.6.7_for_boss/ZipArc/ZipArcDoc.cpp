...
int CZipArcDoc::ModifyMenu(bool bAction)
{
	...
	if (pm && IsMenu(pm->m_hMenu))
		pm->DestroyMenu();
	m_menu.ModifyMenu(iSubMenu, MF_BYPOSITION|MF_POPUP, (UINT_PTR) menu.Detach(), CString((LPCTSTR)IDS_COMMAND));
...
bool CZipArcDoc::UpdateReplaceIndex(ZIP_INDEX_TYPE& uReplaceIndex, const CString &szFileName, int& iLastReplaceAction)
{
	...
	if (m_options.m_bReplace)
	{
		...
		if (idx != ZIP_FILE_INDEX_NOT_FOUND)
		{
			...
			{
				time_t ttime;
				if (!ZipPlatform::GetFileTimes(szFileName, &ttime)
					|| m_zip.GetFileInfo(idx)->GetModificationTime() >= ttime)
					bAdd = false;
				else
					uReplaceIndex = idx;
			}
