...
CProgressBar::CProgressBar(LPCTSTR strMessage, int nSize /*=100*/, 
                           ...
                           int nPane /*=0*/)
{
	Create(NULL, strMessage, nSize, MaxValue, bSmooth, nPane);
}
...
CStatusBar* CProgressBar::GetStatusBar()
{
    ...
    if (m_pWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd)))
    {
        CWnd* pMessageBar = ((CFrameWnd*)m_pWnd)->GetMessageBar();
    ...
    // otherwise traverse children to try and find the status bar...
	    ...
	    return DYNAMIC_DOWNCAST(CStatusBar, 
                                m_pWnd->GetDescendantWindow(AFX_IDW_STATUS_BAR));
}
...
BOOL CProgressBar::Create(CWnd* pWnd, LPCTSTR strMessage, int nSize /*=100*/, 
						  int MaxValue /*=100*/, BOOL bSmooth /*=FALSE*/, 
                          int nPane /*=0*/)
{
	BOOL bSuccess = FALSE;
