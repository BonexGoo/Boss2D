...
RingBuffer* WebRtc_CreateBuffer(size_t element_count, size_t element_size) {
  ...
  self = (RingBuffer*) malloc(sizeof(RingBuffer)); // modified by BOSS, added-code: (RingBuffer*)
  if (!self) {
    return NULL;
  }

  self->data = (char*) malloc(element_count * element_size); // modified by BOSS, added-code: (char*)
  if (!self->data) {
    free(self);
    self = NULL;
    return NULL;
