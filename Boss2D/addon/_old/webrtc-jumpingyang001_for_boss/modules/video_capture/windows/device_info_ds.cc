...
#include BOSS_WEBRTC_U_modules__video_capture__windows__device_info_ds_h //original-code:"modules/video_capture/windows/device_info_ds.h"
...
#include BOSS_WEBRTC_U_modules__video_capture__windows__help_functions_ds_h //original-code:"modules/video_capture/windows/help_functions_ds.h"
#include BOSS_WEBRTC_U_rtc_base__logging_h //original-code:"rtc_base/logging.h"
#include BOSS_WEBRTC_U_rtc_base__stringutils_h //original-code:"rtc_base/stringutils.h"

#include <dvdmedia.h>
// removed by BOSS:#include <streams.h>
...

{
  ...
  for (int32_t tmp = 0; tmp < count; ++tmp) {
    ...
    if (pmt->majortype == MEDIATYPE_Video &&
      ...
      // removed by BOSS: else if (pmt->subtype ==
      // removed by BOSS:            MEDIASUBTYPE_HDYC)  // Seen used by Declink capture cards. Uses
                                     // BT. 709 color. Not entiry correct to use
                                     // UYVY. http://en.wikipedia.org/wiki/YCbCr
      // removed by BOSS: {
      // removed by BOSS:   RTC_LOG(LS_INFO) << "Device support HDYC.";
      // removed by BOSS:   capability.videoType = VideoType::kUYVY;
      // removed by BOSS: }
      else {
        WCHAR strGuid[39];
