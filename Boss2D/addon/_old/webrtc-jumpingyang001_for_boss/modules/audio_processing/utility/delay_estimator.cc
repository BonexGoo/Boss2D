...
int WebRtc_AllocateHistoryBufferMemory(BinaryDelayEstimator* self,
                                       int history_size) {
  BinaryDelayEstimatorFarend* farend = self->farend; // modified by BOSS, original-code: far, replace-code: farend
  // (Re-)Allocate memory for spectrum and history buffers.
  if (history_size != farend->history_size) { // modified by BOSS, original-code: far, replace-code: farend
    // Only update far-end buffers if we need.
    history_size = WebRtc_AllocateFarendBufferMemory(farend, history_size); // modified by BOSS, original-code: far, replace-code: farend
  }
