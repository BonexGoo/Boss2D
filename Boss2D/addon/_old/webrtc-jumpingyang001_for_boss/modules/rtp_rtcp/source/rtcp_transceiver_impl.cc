...
#include BOSS_WEBRTC_U_modules__rtp_rtcp__source__rtcp_transceiver_impl_h //original-code:"modules/rtp_rtcp/source/rtcp_transceiver_impl.h"
...
std::vector<rtcp::ReportBlock_rtcp_BOSS> RtcpTransceiverImpl::CreateReportBlocks( // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
    int64_t now_us) {
  ...
  std::vector<rtcp::ReportBlock_rtcp_BOSS> report_blocks = // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
      config_.receive_statistics->RtcpReportBlocks(
          rtcp::ReceiverReport::kMaxNumberOfReportBlocks);
  uint32_t last_sr = 0;
  uint32_t last_delay = 0;
  for (rtcp::ReportBlock_rtcp_BOSS& report_block : report_blocks) { // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
