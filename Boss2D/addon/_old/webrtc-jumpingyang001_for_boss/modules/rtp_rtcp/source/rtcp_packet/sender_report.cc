...
bool SenderReport::Parse(const CommonHeader& packet) {
  ...
  if (packet.payload_size_bytes() <
      kSenderBaseLength + report_block_count * ReportBlock_rtcp_BOSS::kLength) { // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
  ...
  for (ReportBlock_rtcp_BOSS& block : report_blocks_) { // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
    bool block_parsed = block.Parse(next_block, ReportBlock_rtcp_BOSS::kLength); // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
    RTC_DCHECK(block_parsed);
    next_block += ReportBlock_rtcp_BOSS::kLength; // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
  }
...
bool SenderReport::AddReportBlock(const ReportBlock_rtcp_BOSS& block) { // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
...
bool SenderReport::SetReportBlocks(std::vector<ReportBlock_rtcp_BOSS> blocks) { // modified by BOSS, original-code: ReportBlock, replace-code: ReportBlock_rtcp_BOSS
