...
int WebRtc_AllocateHistoryBufferMemory(BinaryDelayEstimator* self,
                                       int history_size) {
  BinaryDelayEstimatorFarend* _far = self->farend; //modified by BOSS:far를 _far로
  // (Re-)Allocate memory for spectrum and history buffers.
  if (history_size != _far->history_size) { //modified by BOSS:far를 _far로
    // Only update far-end buffers if we need.
    history_size = WebRtc_AllocateFarendBufferMemory(_far, history_size); //modified by BOSS:far를 _far로
  }
