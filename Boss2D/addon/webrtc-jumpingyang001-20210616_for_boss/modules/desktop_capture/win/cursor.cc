...
#include BOSS_WEBRTC_U_modules__desktop_capture__win__scoped_gdi_object_h //original-code:"modules/desktop_capture/win/scoped_gdi_object.h"
...
const int kBytesPerPixel_cursor_BOSS = DesktopFrame::kBytesPerPixel; //added by BOSS:_cursor_BOSS
...
void AlphaMul(uint32_t* data, int width, int height) {
  static_assert(sizeof(uint32_t) == kBytesPerPixel_cursor_BOSS, //added by BOSS:_cursor_BOSS
...
MouseCursor* CreateMouseCursorFromHCursor(HDC dc, HCURSOR cursor) {
  ...
  bmi.bV5Planes = 1;
  bmi.bV5BitCount = kBytesPerPixel_cursor_BOSS * 8; //added by BOSS:_cursor_BOSS
