...
#include BOSS_WEBRTC_U_audio__channel_send_frame_transformer_delegate_h //original-code:"audio/channel_send_frame_transformer_delegate.h"
...
class ChannelSend : public ChannelSendInterface,
 ...
 public:
  ...
  std::vector<ReportBlock_struct_BOSS> GetRemoteRTCPReportBlocks() const override; //added by BOSS:_struct_BOSS
...
void ChannelSend::SetSendAudioLevelIndicationStatus(bool enable, int id) {
  ...
  if (enable) {
    rtp_rtcp_->RegisterRtpHeaderExtension(webrtc::AudioLevel::kUri, id); //modified by BOSS: 'webrtc::' 붙임
  } else {
    rtp_rtcp_->DeregisterSendRtpHeaderExtension(webrtc::AudioLevel::kUri); //modified by BOSS: 'webrtc::' 붙임
  }
}
...
std::vector<ReportBlock_struct_BOSS> ChannelSend::GetRemoteRTCPReportBlocks() const { //added by BOSS:_struct_BOSS
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  // Get the report blocks from the latest received RTCP Sender or Receiver
  // Report. Each element in the vector contains the sender's SSRC and a
  // report block according to RFC 3550.
  std::vector<ReportBlock_struct_BOSS> report_blocks; //added by BOSS:_struct_BOSS
  for (const ReportBlockData& data : rtp_rtcp_->GetLatestReportBlockData()) {
    ReportBlock_struct_BOSS report_block; //added by BOSS:_struct_BOSS
