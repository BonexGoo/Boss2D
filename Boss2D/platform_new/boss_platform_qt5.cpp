...
#include "boss_platform_qt5.hpp"
    ...
    void CanvasClass::UnbindCore()
    {
        ...
        int main(int argc, char* argv[])
        {
            ...
            #endif
            {
                ...
                QSurfaceFormat OneFormat;
                OneFormat.setDepthBufferSize(24);
                // Request OpenGL 3.3 core or OpenGL ES 3.0
                if(QOpenGLContext::openGLModuleType() == QOpenGLContext::LibGL)
                {
                    OneFormat.setVersion(3, 3);
                    OneFormat.setProfile(QSurfaceFormat::CoreProfile);
                }
                else OneFormat.setVersion(3, 0);
                QSurfaceFormat::setDefaultFormat(OneFormat);
    ...
    namespace BOSS
    {
        ...
        void Platform::Graphics::DrawTexture(id_surface surface_or_screen, float x, float y, float w, float h,
            id_texture_read texture, float tx, float ty, float tw, float th, orientationtype ori, bool antialiasing, Color color)
        {
            BOSS_ASSERT("호출시점이 적절하지 않습니다", CanvasClass::get());
            BOSS_ASSERT("본 함수를 호출하기 전에 BeginGL()을 호출하여야 안전합니다", g_isBeginGL);
                ...
                OpenGLPrivate::ST().DrawTexture(surface_or_screen, Rect(x, y, x + w, y + h),
        ...
        void Platform::Graphics::DrawPolyImage(id_surface surface_or_screen, float x, float y, const Point (&ps)[3],
            id_image_read image, const Point (&ips)[3], const Color (&colors)[3])
        {
            BOSS_ASSERT("호출시점이 적절하지 않습니다", CanvasClass::get());
            BOSS_ASSERT("본 함수를 호출하기 전에 BeginGL()을 호출하여야 안전합니다", g_isBeginGL);
            BOSS_ASSERT("image파라미터가 nullptr입니다", image);
                ...
                OpenGLPrivate::ST().DrawPixmap(surface_or_screen, x, y, ps,
                    *((const PixmapPrivate*) image), ips, colors);
            #endif
        }
            ...
            void Render(sint32 x, sint32 y, wchars string, sint32 count, ColorPrivate color, float justifyrate)
            {
                ...
                if(Platform::Graphics::BeginGL())
                {
                    ...
                    for(sint32 i = 0; i < count; ++i)
                    {
                        ...
                        if(CurCodeData)
                        {
                            Platform::Graphics::DrawTexture(nullptr,
                                x + CurCodeData->mRenderPos.x * justifyrate, y + CurCodeData->mRenderPos.y,
                                CurCodeData->mUVSize.w * justifyrate, CurCodeData->mUVSize.h,
                                (id_texture_read) CurCodeData->mRefTexture,
                                CurCodeData->mUVPos.x, CurCodeData->mUVPos.y, CurCodeData->mUVSize.w, CurCodeData->mUVSize.h,
                                orientationtype_normal0, false, CurColor);
        ...
        void Platform::Graphics::DrawSurface(float x, float y, float w, float h, id_surface_read surface, float sx, float sy, float sw, float sh)
        {
            BOSS_ASSERT("호출시점이 적절하지 않습니다", CanvasClass::get());
            #ifndef BOSS_SILENT_NIGHT_IS_ENABLED
                if(!surface) return;
        ...
        void Platform::Graphics::DrawSurfaceToSurface(id_surface dest, float dx, float dy, float dw, float dh,
            id_surface_read src, float sx, float sy, float sw, float sh, orientationtype ori, bool antialiasing, Color color)
        {
            BOSS_ASSERT("호출시점이 적절하지 않습니다", CanvasClass::get());
            BOSS_ASSERT("본 함수를 호출하기 전에 BeginGL()을 호출하여야 안전합니다", g_isBeginGL);
            #ifndef BOSS_SILENT_NIGHT_IS_ENABLED
                ...
                OpenGLPrivate::ST().DrawSurface(dest, Rect(dx, dy, dx + dw, dy + dh),
                    ((const SurfaceClass*) src)->GetLastImage(), Rect(sx, sy, sx + sw, sy + sh), color, ori, antialiasing);
            #endif
        }
