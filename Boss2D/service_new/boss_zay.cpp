...
namespace BOSS
{
        ...
        if(Platform::Graphics::BeginGL())
        {
            ...
            if(build != Image::Build::Null)
            {
                if(id_image_read RebuildImage = image.GetBuildImage(DstWidth, DstHeight, build))
                    Platform::Graphics::DrawPolyImage(m_ref_surface, LastClip.l, LastClip.t, ps,
                        RebuildImage, ips, colors);
                if(m_updater && !image.IsBuildFinished())
                    m_updater->RepaintOnce();
            }
            else Platform::Graphics::DrawPolyImage(m_ref_surface, LastClip.l, LastClip.t, ps,
                image.GetImage(), ips, colors);
    ...
    void ZayPanel::sub(chars uigroup, id_surface surface) const
    {
        ...
        if(surface)
        {
            ...
            Platform::Graphics::DrawSurfaceToSurface(m_ref_surface, LastClip.l, LastClip.t, DstWidth, DstHeight,
                surface, 0, 0, SurfaceWidth, SurfaceHeight, orientationtype_normal0, false);
        }
    }
