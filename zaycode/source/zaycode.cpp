...
ZAY_DECLARE_VIEW_CLASS("zaycodeView", zaycodeData)
...
void zaycodeData::ProjectLoad(chars dirpath)
{
    ...
    {
        zaycodeData& Self = *((zaycodeData*) self);
        ...
        Platform::Utility::Sleep(500, true, true, true);
        return -1;
    }, (buffer) this);
}

void zaycodeData::RenderSourceView(ZayPanel& panel)
{
    if(mFileFocus)
    {
...
void zaycodeData::RenderAPIView(ZayPanel& panel)
{
...
void zaycodeData::RenderChecklistView(ZayPanel& panel)
{
    const sint32 CheckBoxAreaSize = 10 + CheckBoxSize;
...
void zaycodeData::CollectSourceFile(chars dirpath)
{
    Platform::File::Search(dirpath, [](chars itemname, payload data)->void
    {
        auto& Self = *((zaycodeData*) data);
...
Image& zaycodeData::GetUserPic(chars author)
{
...
String& zaycodeData::GetUserName(chars author)
{
