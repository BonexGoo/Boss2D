...
// ZEDocument
////////////////////////////////////////////////////////////////////////////////
ZEDocument::ZEDocument() : ZaySonDocument("d")
{
    mReserverTask = nullptr;
}

ZEDocument::~ZEDocument()
{
    Tasking::Release(mReserverTask, true);
}

ZEDocument::ZEDocument(ZEDocument&& rhs) : ZaySonDocument("d")
{
    operator=(ToReference(rhs));
}

ZEDocument& ZEDocument::operator=(ZEDocument&& rhs)
{
...
void ZEDocument::PostProcess(const String key, const String value)
{
    // 마지막 키명 추출
...
void ZEDocument::RemoveCB(chars itemname, payload data)
{
...
////////////////////////////////////////////////////////////////////////////////
// ZEDocument::DownloadReserver
////////////////////////////////////////////////////////////////////////////////
ZEDocument::DownloadReserver::DownloadReserver()
{
    mHtml = false;
}
...
sint32 ZEDocument::DownloadReserver::OnTask(buffer& self, Queue<buffer>& query, Queue<buffer>& answer, id_common common)
{
...
// ZEWidgetDOM
////////////////////////////////////////////////////////////////////////////////
void ZEWidgetDOM::Add(chars variable, chars formula)
{
    auto& Self = ST();
    Self.mDocument.Add(variable, formula);
}

void ZEWidgetDOM::AddJson(const Context& json)
{
    auto& Self = ST();
    Self.mDocument.AddJson(json);
}

void ZEWidgetDOM::AddFlush()
{
    auto& Self = ST();
...
void ZEWidgetDOM::ReserverFlush()
{
    auto& Self = ST();
    Self.mDocument.ReserverFlush();
}

void ZEWidgetDOM::Update(chars variable, chars formula)
{
    auto& Self = ST();
...
void ZEWidgetDOM::BindPipe(id_pipe pipe)
{
    auto& Self = ST();
...
void ZEWidgetDOM::UnbindPipe(id_pipe pipe)
{
    auto& Self = ST();
    Self.mPipeMap.Remove(PtrToUint64(pipe));
}

void ZEWidgetDOM::SendFlush()
{
...
void ZEWidgetDOM::SendToPipe(id_pipe pipe, const Solver* solver)
{
    Context Json;
...
// ZEWidget
////////////////////////////////////////////////////////////////////////////////
ZaySon& ZEWidget::Init(chars viewname, chars assetname, ResourceCB cb)
{
...
void ZEWidget::Reload(chars assetname)
{
            ...
            [](payload data)->void
            {
                ...
                if(Self->mLastProcedureTime + 1000 < CurUpdateCheckTime)
                {
                    ...
                    if(FileInfo == -1)
                    {
                        ...
                        {
                            ZEWidgetDOM::UnbindPipe(Self->mPipe);
                    ...
                    else
                    {
                        if(Self->mPipeModifyTime != ModifyTime)
                        {
                            ...
                            ZEWidgetDOM::BindPipe(Self->mPipe);
                            // 그동안 쌓인 송신내용
...
void ZEWidget::Render(ZayPanel& panel)
{
    mZaySon.Render(panel);
}

bool ZEWidget::TickOnce()
{
...
ZEWidget::ZEWidget(ZEWidget&& rhs)
{
    operator=(ToReference(rhs));
}

ZEWidget& ZEWidget::operator=(ZEWidget&& rhs)
{
...
void ZEWidget::BuildComponents(chars viewname, ZaySonInterface& interface, ResourceCB* pcb)
{
...
void ZEWidget::BuildGlues(chars viewname, ZaySonInterface& interface, ResourceCB* pcb)
{
